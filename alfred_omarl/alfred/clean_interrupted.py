...
def get_clean_interrupted_args():
    ...
    parser.add_argument('--make_preempted_flag', type=parse_bool, default=False, help="Only removes CRASH.txt if present and creates PREEMPTED instead of UNHATCHED")

    parser.add_argument('--root_dir', default=None, type=str)
    return parser.parse_args()


def clean_interrupted(from_file, storage_name, clean_crashes, clean_crashes_only, ask_for_validation, make_preempted_flag, logger, root_dir):
    ...
    for storage_dir in storage_dirs:
        ...
        if len(seeds_to_clean) != 0:
            ...
            for seed_dir in seeds_to_clean:
                ...
                if make_preempted_flag:
                    if (seed_dir/'CRASH.txt').exists():
                        os.remove(str(seed_dir/'CRASH.txt'))
                    open(str(seed_dir / 'PREEMPTED'), 'w+').close()
                else:
